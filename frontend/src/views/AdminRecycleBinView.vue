...
async function load() {
  loading.value = true
  try {
    const { data } = await http.get<ApiResponse<any>>('/admin/recycle-bin/products', {
...
async function restore(id: number) {
  try {
    await http.post(`/admin/recycle-bin/products/${id}/restore`)
...
async function purge(id: number) {
  try {
    await http.delete(`/admin/recycle-bin/products/${id}`)
...
function onSelect(rows: any[]) { /* 预留批量操作 */ }
